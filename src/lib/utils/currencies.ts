...
// USD, EUR, GBP are prioritized at the top
...
// All other currencies (alphabetically by code)
...
// Combined list with priority currencies first
...
export function searchCurrencies(query: string): Currency[] {
  ...
  const exactMatches = ALL_CURRENCIES.filter(
    (c) => c.code.toLowerCase() === lowerQuery
  );

  // Then, find currencies that start with the query
  const startsWithMatches = ALL_CURRENCIES.filter(
    (c) =>
      c.code.toLowerCase().startsWith(lowerQuery) ||
      c.name.toLowerCase().startsWith(lowerQuery)
  );

  // Then, find currencies that contain the query
  const containsMatches = ALL_CURRENCIES.filter(
    (c) =>
      c.code.toLowerCase().includes(lowerQuery) ||
      c.name.toLowerCase().includes(lowerQuery)
  );

  // Combine and dedupe, prioritizing exact > starts with > contains
  const seen = new Set<string>();
  const results: Currency[] = [];

  for (const currency of [...exactMatches, ...startsWithMatches, ...containsMatches]) {
